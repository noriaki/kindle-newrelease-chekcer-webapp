...
const reducer = (ret, item) => {
  ...
  if (asin) {
    ...
    ret[asin] = {
      ...
      authors: arrayify(dig(item, 'ItemAttributes', 'Author')),
...
const getSeries = async (asins, retry = 2) => {
  ...
  if (items) {
    ...
    if (error) {
      ...
      if (error.Code === 'RequestThrottled' && retry >= 0) {
        await sleep(1000);
        return await getSeries(asins, retry - 1);
      } else if (error.Code === 'RequestThrottled' && retry < 0) {
        return { error: 'RequestThrottled' };
        ...
        const results = await Promise.all(asinArr.map(asin => getSeries(asin)));
